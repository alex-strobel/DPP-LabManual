...
Basically, follow the first link above, click on the [download R](https://cran.r-project.org/mirrors.html) link in the first paragraph under "Getting Started", choose a download mirror (e.g., https://ftp.fau.de/cran/), and then download the R distribution for your operating system and install it. Then follow the second link, choose the appropriate operating system, download and install Rstudio.
If installing a new R/RStudio version after you have already worked with previous versions, you may want to resort to the structions given below under [Where have all my packages gone?](#where-have-all-my-packages-gone).
