...
- **Norms**
  ...
  - For intersecting subgroups (e.g. women with high school diploma), there should be an additional folder which contains the name of both groups (e.g. education_gender) which contains the files for the subgroups, again with the labels separated by an underscore (e.g. women_abitur). **NOTE**: Because it is assumed, that underscores separate the names of subgroupd, the names of an individual subgroup themselves cannot contain underscores!
