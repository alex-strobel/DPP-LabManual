...
6. It is highly recommended to commit every change made separately so that the commit is labeled approprately. So if you, e.g., make a change in a script: commit it and label it "Update My-Script.R" 8in fact, GitHub Desktop will suggest exactly this label). You put some data in a newly created folder: commit it and label it. If commit both things at once before committing them, it may be hard to find an appropriate label because the label will be attached to both the script and the folder.
